...
export default function CrearMatriculaModal({ onClose }) {
  ...
  useEffect(() => {
    const cargarDatos = async () => {
      ...
      // Mallas con nombre completo (Carrera + Ciclo)
      const mallasConNombre = allMallas.map(m => ({
        ...m,
        nombreCompleto: `${cMap[m.carreraId] || "Sin carrera"} - Ciclo ${m.ciclo}`,
      }));
  ...
  const crearMatricula = async () => {
    ...
    try {
      await MatriculaActions.crearMatricula(
        {
          estudianteId,
          mallaId,
        },
  ...
  return (
    <Modal isOpen={true} onClose={onClose} isCentered>
      <ModalOverlay />
      <ModalContent>
        <ModalHeader>Crear Nueva Matrícula</ModalHeader>
        <ModalCloseButton />
        <ModalBody>
          <FormControl mb={3}>
