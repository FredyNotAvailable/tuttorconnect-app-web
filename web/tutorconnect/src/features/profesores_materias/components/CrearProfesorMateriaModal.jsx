...
export default function CrearProfesorMateriaModal({ onClose }) {
  ...
  const [ciclo, setCiclo] = useState(""); // 🔹 nuevo estado
  ...
  useEffect(() => {
    const cargarDatos = async () => {
      ...
      const allMateriasMalla =
        await MateriaMallaRepository.getAllMateriasMalla();
  ...
  const crearProfesorMateria = async () => {
    ...
    try {
      await ProfesorMateriaActions.crearProfesorMateria({ profesorId, materiaId, mallaId }, toast );
      onClose();
    } catch (error) {
      toast({
  ...
  // 🔹 Ciclos disponibles fijos del 1 al 10
  const ciclosOptions = Array.from({ length: 10 }, (_, i) => ({
    value: i + 1,
    label: `Ciclo ${i + 1}`,
  }));

  // 🔹 Filtrar mallas por ciclo seleccionado
  ...
  // Filtrar materias según malla seleccionada
        ...
        <ModalBody>
          ...
          <FormControl mb={3}>
            ...
            <Select
              ...
              placeholder={
                ciclo ? "Seleccione una malla por ciclo" : "Seleccione primero un ciclo"
              }
              isClearable
              isSearchable
              isDisabled={!ciclo}
          ...
          <FormControl mb={3}>
            ...
            <Select
              ...
              placeholder={
                mallaId
                  ? "Seleccione una materia de la malla"
                  : "Seleccione primero una malla"
              }
              isClearable
              isSearchable
              isDisabled={!mallaId}
